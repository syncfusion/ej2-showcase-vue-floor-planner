<template>
  <div>
    ...
    <ejs-contextmenu id="arrangeContextMenu" :items="dropDownDataSources.arrangeMenuItems" :animationSettings="dialogAnimationSettings" @open="arrangeContextMenuOpen"
      cssClass="arrangeMenu" :beforeClose="arrangeMenuBeforeClose" @select="menuClick($event)" >
    </ejs-contextmenu>
        ...
        <div class="db-header-container">
          ...
          <div class="db-toolbar-editor">
            <div class="db-toolbar-container">
              <ejs-toolbar id="toolbarEditor" ref="toolbarEditorRef" overflowMode="Scrollable" @clicked="toolbarEditorClick">
                <e-items>
                  <e-item
                  ...
                  ></e-item>
                  <e-item type="Separator" :visible="false"></e-item>
                  <e-item align="Right" cssClass="tb-item-end tb-zoom-dropdown-btn" :template="'dropdownTemplate'"  >
                  </e-item>
                  <template v-slot:dropdownTemplate>
                      <div>
                          <ejs-dropdownbutton id="btnZoomIncrement" :items="menuItems" @select="zoomChange($event)"  :content="selectedItem.scrollSettings.currentZoom"></ejs-dropdownbutton>
                      </div>
                  </template>
                  <e-item tooltipText="Hide property" align="Right" :template= "'hidePropertyTemplate'">
                  </e-item>
                  <template v-slot:hidePropertyTemplate>
                    <div>
                        <ejs-button  id="hideProperty" @click="hideElements()" iconCss="sf-icon-properties" isPrimary="true"></ejs-button>
                    </div>
                  </template>
                </e-items>
              </ejs-toolbar>
          ...
          <div class="db-diagram-container">
            <div id="diagramContainerDiv" class="db-current-diagram-container">
              <ejs-diagram
                ...
                :created="() => diagramEvents.created()"
                :selectionChange="(args) => diagramEvents.selectionChange(args)"
                :positionChange="(args) => diagramEvents.nodePositionChange(args)"
                :sizeChange="(args) => diagramEvents.nodeSizeChange(args)"
                :rotateChange="(args) => diagramEvents.nodeRotationChange(args)"
                :dragEnter="(args) => diagramEvents.dragEnter(args)"
                :historyChange="(args) => diagramEvents.historyChange(args)"
                :scrollChange="(args) => this.scrollChange(args)"
          ...
          <div id="generalDiagramContainer" class="db-general-diagram-prop-container">
            <div id='diagramPropertyContainer' class="db-diagram-prop-container">
              <div class="row db-prop-header-text">
                Page Settings
                  <ejs-button class="close"  @click="hideElements('hide-properties',diagram)"><i style="width: 20px;height: 20px;font-size:20px" class="sf-icon-close"></i></ejs-button>
              </div>
              ...
              <div class="col-xs-6 db-col-left">
                <ejs-button id="pagePortrait" isPrimary="true" isToggle="true" style="font-size: 10px;"
                  cssClass="e-flat e-primary"  iconCss="sf-icon-portrait"  name="pageSettings"
                   @click='diagramPropertyBinding.pageOrientationChange($event)'>Portrait</ejs-button>
              </div>
              <div class="col-xs-6 db-col-right" >
                  <ejs-button id="pageLandscape" isPrimary="true" isToggle="true" style="font-size: 10px;"
                    cssClass="e-flat e-primary e-active" iconCss="sf-icon-landscape" name="pageSettings"
                     @click='diagramPropertyBinding.pageOrientationChange($event)'>LandScape</ejs-button>
              </div>
              </div>
              ...
              <div class="row db-prop-row">
                ...
                <div class="col-xs-2 db-col-left" style="width: 20px;padding-left:7px">
                  <ejs-button style="height: 26px; background-color: #ffff;" 
                    id='aspectRatioBtn'  v-on:click="aspectClick($event)"
              ...
              <div class="row db-prop-row">
                ...
                <div class="row db-prop-row">
                  <div class="col-xs-6 db-col-left">
                    ...
                    id="insertlink"
                    iconCss= 'sf-icon-insert_link' tooltipText='Insert Link' cssClass= 'e-outline' isPrimary=true  @click="toolbarInsertClick"
                    >Insert link</ejs-button>
                  </div>
                  <div class="col-xs-6 db-col-left">
                    <ejs-button style="font-size: 10px;margin-left: 5px;width:110px;padding-left:3px"
                    id="insertimage" iconCss= 'sf-icon-insert_image' tooltipText='Insert Image' cssClass= 'e-outline' isPrimary=true @click="toolbarInsertClick">Insert image</ejs-button>
                  </div>
                </div>
                    ...
                    <div class="row">
                      <div class="col-xs-6 db-col-right" style="width: 77px;margin-right: 0px;">
                        <ejs-dropdownlist id="nodeBorderStyle" v-model="selectedItem.nodeProperties.strokeStyle" :dataSource="dropDownDataSources.borderStyles"
                          popupWidth="160px" :fields="dropdownListFields" :itemTemplate="'iTemplate'" :valueTemplate="'vTemplate'">
              ...
              <div id='textPropertyContainer' class="db-text-prop-container" style="display:none">
                ...
                <div class="row db-prop-row">
                  <div class="col-xs-6 db-col-left" id="textPositionDiv" style="width:148px;">
                    <ejs-dropdownlist id="ddlTextPosition" ref="ddlTextPositionRef" index=0 :dataSource="selectedItem.textProperties.textPositionDataSource" :value="selectedItem.textProperties.textPosition"
                    ...
                    <div>
                      <ejs-toolbar
                        ...
                        overflowMode="Scrollable"
                         @clicked="diagramPropertyBinding.toolbarTextSubAlignChange($event)"
                      >
                        <e-items>
                          <e-item
                ...
                <div class="row db-prop-row" id='toolbarTextAlignmentDiv'>
                    ...
                    overflowMode="Scrollable"
                     @clicked="diagramPropertyBinding.toolbarTextAlignChange($event)"
                  >
                    <e-items>
                      <e-item
...
// Define gridlines interval outside the setup function for reusability
const interval = [
...
export default defineComponent({
  ...
  setup() {
    ...
    const dlgTarget = ref(document.body);
    const dialogPosition = ref({ X: 100, Y: 112 });
    const dialogVisibility = ref(false);
    const isModalDialog = ref(false);
    const dialogAnimationSettings = ref({ effect: "None" });
    const path = reactive({
    ...
    const toolbarEditor = ref();
    const exportDialog = ref();
    const hyperlinkDialog = ref();
    const defaultupload = ref();
    ...
    const setConnectorDefaults = (connector) => {
      let connector1 = {
        annotations: [{ content: "", style: { fill: "transparent" } }],
        style: { strokeWidth: 2 },
      };
      return connector1;
    };
   ...
   const contextMenu = reactive({
    ...
    const collectionChange = ref({});
    const drop = ref({});
    const getCustomTool = ref({});
  ...
  methods: {
    onWindowLoad() {
      this.diagram = this.$refs.diagramInstanceRef.ej2Instances;
      this.toolbarEditor = this.$refs.toolbarEditorRef.ej2Instances;
      btnZoomIncrement = this.$refs.toolbarEditorRef.ej2Instances;
    ...
    // Handle changes in the scroll state and update the zoom content when scrolling in the diagram.
    scrollChange(args) {
    ...
    // Arrange Menu Items before Opening the context Menu bar Items,
    ...
    // Sets all child elements of a menu to be displayed as block
    ...
    // Arrange Menu Items before closing the context Menu bar Items
    ...
    // To define the behavior and options of the context menu for Design Menu tab
    ...
    // Updates the display and export filename to match the diagram's editable name
    diagramNameChange() {
    ...
    // Add keyboard shortcut key for a specific action 
    ...
    enableMenuItems(itemText) {
      let selectedDiagram = this.diagram.ej2_instances ? this.diagram.ej2_instances[0] : this.diagram;
      ...
      if (itemText) {
        ...
        if (!(this.diagram.commandHandler.clipboardData.pasteIndex !== undefined && 
        this.diagram.commandHandler.clipboardData.clipObject !== undefined) && itemText === "Paste") {
          return true;
        }

        if (itemText === "Undo" && this.diagram.historyManager.undoStack.length === 0) {
          return true;
        }
        if (itemText === "Redo" && this.diagram.historyManager.redoStack.length === 0) {
          return true;
        }
    ...
    // Handle mouseover events on the menu bar items and manage their behavior, including toggling dropdown menus and applying styles.
    menumouseover(args) {
      ...
      if (target && (target.className === "e-control e-dropdown-btn e-lib e-btn db-dropdown-menu" ||
        ...
        if (this.buttonInstance && this.buttonInstance.id !== target.id) {
          if (this.buttonInstance.getPopUpElement() && this.buttonInstance.getPopUpElement()
        ...
        if (button1 && button1.getPopUpElement() && button1.getPopUpElement().classList.contains("e-popup-close")) {
          button1.toggle();
        ...
        if (
          ...
          if (this.buttonInstance && this.buttonInstance.getPopUpElement() &&
            this.buttonInstance.getPopUpElement().classList.contains("e-popup-open")) {
            this.buttonInstance.toggle();
            let buttonElement = document.getElementById(
              this.buttonInstance.element.id
            );
    ...
    //Changes the value of zoom dropdown in toolbar
    zoomChange(args) {
      let diagram = this.selectedItem.diagram || document.getElementById("diagram").ej2_instances[0];
    ...
    hideElements() {
      let diagramContainer = document.getElementsByClassName("diagrambuilder-container")[0];
    ...
    // Toggles aspect ratio locking for diagram elements
    aspectClick() {
      let isAspect = true;
      let aspectRatioBtn =
        document.getElementById("aspectRatioBtn").ej2_instances[0];
      if (
        document.getElementById("aspectRatioBtn").classList.contains("e-active")
      ) {
    ...
    // Handle click events on the menu items and execute corresponding actions in the diagram.
    menuClick(args) {
      ...
      let command = args.item.text.replace(/[" "]/g, "").toLowerCase();
      switch (command) {
        case "new":
          this.diagram.clear();
          break;
        ...
        case "fittowidth":
          ...
          break;
        case "showrulers":
          this.diagram.rulerSettings.showRulers =
            !this.diagram.rulerSettings.showRulers;
          args.item.iconCss = args.item.iconCss ? "" : "sf-icon-check-tick";
          break;
        case "pagebreak":
          ...
          if (args.item.iconCss) {
            ...
            this.diagram.pageSettings.showPageBreaks = false;
        ...
        case "landscape":
          ...
          this.diagram.pageSettings.orientation = "Landscape";
        ...
        case "portrait":
          ...
          args.item.iconCss = "sf-icon-check-tick";
          this.diagram.pageSettings.orientation = "Portrait";
    ...
    // Updates diagram name on key press.
    ...
    // Handles insert link and image actions based on menu clicks.
    menuClick1(args) {
      let buttonElement = document.getElementsByClassName("e-btn-hover")[0];
      if (buttonElement) {
        buttonElement.classList.remove("e-btn-hover");
    ...
    // Opens an upload box with specified allowed file extensions.
    openUploadBox(isOpen, extensionType) {
    ...
    // Initiates a download of the diagram in JSON format
    download(data) {
    ...
    // Assigns the current diagram object to the selected item
    generateDiagram() {
      this.selectedItem.diagram = this.diagram;
    },
    
    // Returns true, allowing command execution.
    canExecute() {
      return true;
    },

    // Handle click event on the toolbar editor.
    toolbarEditorClick(args) {
      const diagram = document.getElementById("diagram").ej2_instances[0];
      const commandType = (args.item.tooltipText || "")
        .replace(/[' ']/g, "")
        .toLowerCase();
      const zoomCurrentValue =
      ...
      switch (commandType) {
        ...
        case "zoomin(ctrl++)":
          ...
          zoomCurrentValue.content = `${(
            this.diagram.scrollSettings.currentZoom * 100
          ).toFixed()}%`;
          break;
        case "zoomout(ctrl+-)":
        ...
        case "save":
          ...
          break;
        case "fillcolor":
          const objColor = this.diagram.selectedItems.nodes[0]
            ? "nodeFillColor"
            : "lineColor";
          this.selectedItem.utilityMethods.showColorPicker(
            objColor,
            "tb-item-stroke"
          );
          break;
        case "fontcolor":
          if (this.diagram.selectedItems.nodes[0]?.annotations) {
            this.selectedItem.utilityMethods.showColorPicker(
              "textColor",
              "tb-item-stroke"
            );
        ...
        case "alignmiddle":
        case "aligncenter":
          const alignType = commandType.replace("align", "");
          const alignType1 =
    ...
    // Flips selected diagram objects horizontally or vertically.
    flipObjects(flipType, diagram) {
      var selectedObjects = diagram.selectedItems.nodes.concat(
        diagram.selectedItems.connectors
      );
      for (let i = 0; i < selectedObjects.length; i++) {
        selectedObjects[i].flip =
          flipType === "horizontalflip" ? "Horizontal" : "Vertical";
      }
      diagram.dataBind();
    },

    // Processes successful file uploads, handling formats like JSON and images
    ...
    setImage(event) {
      let node = this.selectedItem.diagram.selectedItems.nodes[0];
      node.shape = {
        type: "Image",
    ...
    // Loads a diagram from a JSON file's content
    loadDiagram(event) {
    ...
    // Returns button configurations for various dialogs
    ...
    // Provides button settings for the upload dialog
    getUploadButtons() {
      let buttons = [];
      buttons.push({
        click: this.btnCancelClick.bind(this),
    ...
    // Initiates diagram export based on selected settings
    btnExportClick() {
      let diagram = this.selectedItem.diagram;
    ...
    // Initiates printing of the current diagram
    btnPrintClick() {
    ...
    // Cancels dialog actions and hides dialog based on context
    btnCancelClick(args) {
      let key = args.target.offsetParent.id;
      switch (key) {
        case "exportDialog":
          this.exportDialog.hide();
          break;
    ...
    // Applies a hyperlink to the selected node's annotations and updates the diagram
    btnHyperLink() {
      let node = this.selectedItem.diagram.selectedItems.nodes[0];
    ...
    // Configures the tooltip for a node based on its hyperlink
    ...
    //Enables or disables interaction for selected nodes or connectors
    lockObject() {
  ...
  provide: { diagram: [UndoRedo, PrintAndExport, Snapping] },
});
</script>
